...
class BaseComputerTool:
    ...
    _screenshotDelay = 2.0
    _scalingEnabled = True

    @property
    def options(self) -> ComputerToolOptions:
        """Return the display options for this tool."""
        width, height = self.scaleCoordinates(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the computer tool with display and user settings."""
        super().__init__()
        ...
        if username:
            self._displayPrefix = f"sudo -u {username} env DISPLAY=:{self.display_num} "
        else:
            self._displayPrefix = f"DISPLAY=:{self.display_num} "

        self.xdotool = f"{self._displayPrefix}xdotool"
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """Handle a computer action for the 2024-10-22 API version."""
        if action in ("mouse_move", "left_click_drag"):
            ...
            if action == "left_click_drag":
                ...
                start_x, start_y = self.validateAndGetCoordinates(start_coordinate)
                end_x, end_y = self.validateAndGetCoordinates(coordinate)
                command_parts = [
                    self.xdotool,
                ...
                x, y = self.validateAndGetCoordinates(coordinate)
    ...
    def validateAndGetCoordinates(
        self,
        coordinate: tuple[int, int] | None = None,
    ):
        """Validate a coordinate pair and return scaled coordinates."""
        if not isinstance(coordinate, list) or len(coordinate) != 2:
            raise ToolError(f"{coordinate} must be a tuple of length 2")
        if not all(isinstance(i, int) and i >= 0 for i in coordinate):
            raise ToolError(f"{coordinate} must be a tuple of non-negative ints")

        return self.scaleCoordinates(ScalingSource.API, coordinate[0], coordinate[1])
    ...
    async def screenshot(self):
        ...
        if shutil.which("gnome-screenshot"):
            screenshot_cmd = f"{self._displayPrefix}gnome-screenshot -f {path} -p"
        else:
            screenshot_cmd = f"{self._displayPrefix}scrot -p {path}"
    ...
    async def shell(self, command: str, take_screenshot=True) -> ToolResult:
        ...
        if take_screenshot:
            await asyncio.sleep(self._screenshotDelay)
            base64_image = (await self.screenshot()).base64_image

        return ToolResult(output=stdout, error=stderr, base64_image=base64_image)

    def scaleCoordinates(
        self,
...
class ComputerTool20241022(BaseComputerTool, BaseAnthropicTool):
    ...
    def toParams(self) -> BetaToolComputerUse20241022Param:
...
class ComputerTool20250124(BaseComputerTool, BaseAnthropicTool):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """Handle a computer action for the 2025-01-24 API version."""
...
class ComputerTool20251124(ComputerTool20250124):
    """Computer tool implementation for the 2025-11-24 API version with zoom support."""
    ...
    def options(
        ...
        """Return the display options including zoom enablement."""
        return {
            **super().options,
            "enable_zoom": True,
        }
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """Handle a computer action for the 2025-11-24 API version."""
        if action == "zoom":
            if (
                region is None
            ...
            x0, y0, x1, y1 = region
            x0, y0 = self.scaleCoordinates(ScalingSource.API, x0, y0)
            x1, y1 = self.scaleCoordinates(ScalingSource.API, x1, y1)
